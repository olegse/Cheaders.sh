...
#
# write include statements and header file
#
# usage: editc FILE
#
# variables:		fn					visible across the calls
#								headers[]		include statements	
#
function editc() {

	fnc				# get list of the all function calls
	for fn in ${functions[@]}
	do
		# debug
		if [ $debug ]
		...
		do
			...
			then
				...
				then
					...
					sed --in-place=.bak "/\<$fn\>/ c\\$declaration" $header
					continue
				fi
			fi
